...
class tt3(Metric):
    ...
    def __call__(self, y_pred: Tensor, y_true: Tensor) -> np.ndarray:
       ...
       # self.set_pred = y_pred

       # df_preds = pd.DataFrame(self.set_pred, columns = ['preds_col'])
        
      #  Final_DataFrame = pd.concat([train_df, df_preds], axis=1)
      #  df_black = Final_DataFrame[Final_DataFrame['race']=="African-American"]
       # [[tn_black , fp_black],[fn_black , tp_black]]  = confusion_matrix(df_black['two_year_recid'], df_black['preds_col'])
      #  df_Caucasian = Final_DataFrame[Final_DataFrame['race']=="Caucasian"]
      #  [[tn_Caucasian , fp_Caucasian],[fn_Caucasian , tp_Caucasian]]  = confusion_matrix(df_Caucasian['two_year_recid'], df_Caucasian['preds_col'])
      #  Fairness_False_positive_rate_black_Caucasian = (fp_black/(fp_black+tn_black)) - (fp_Caucasian/(fp_Caucasian+tn_Caucasian))
        return np.array(tt) #Fairness_False_positive_rate_black_Caucasian


class tt4(Metric):
    def __init__(self, top_k: int = 1):
        super(tt4, self).__init__()
        self.top_k = top_k
        self.correct_count = 0
        #self.total_count = 0
        #  metric name needs to be defined
    ...
    def reset(self):
        self.correct_count = 0
       # self.total_count = 0
        self.path_1 = 'data_prepration/data_sets/train_df_compas.csv'
        self.train_df = pd.read_csv(self.path_1)
    ...
    def __call__(self, y_pred: Tensor, y_true: Tensor) -> np.ndarray:
       ...
       # self.set_pred = y_pred
       #path_1 = 'data_prepration/data_sets/train_df_compas.csv'
       # train_df = pd.read_csv(self.path_1)
      #  df_preds = pd.DataFrame(total_count.item(), columns = ['preds_col'])
        ...
        #Fairness_False_positive_rate_black_Caucasian = (fp_black/(fp_black+tn_black)) - (fp_Caucasian/(fp_Caucasian+tn_Caucasian))
        return np.array(total_count[2].detach()) #df_preds[2] # np.array(total_count[2]) #Fairness_False_positive_rate_black_Caucasian



class tt5(Metric):
    def __init__(self, top_k: int = 1):
        super(tt5, self).__init__()
        self.top_k = top_k
        self.correct_count = 0
        #self.total_count = 0
        #  metric name needs to be defined
    ...
    def __call__(self, y_pred: Tensor, y_true: Tensor) -> np.ndarray:
      ...
      #  df_preds = pd.DataFrame(total_count.item(), columns = ['preds_col'])
        ...
        #Fairness_False_positive_rate_black_Caucasian = (fp_black/(fp_black+tn_black)) - (fp_Caucasian/(fp_Caucasian+tn_Caucasian))
        return np.array(total_count)[2] #df_preds[2] # np.array(total_count[2]) #Fairness_False_positive_rate_black_Caucasian


class tt6(Metric):
    def __init__(self, top_k: int = 1):
        super(tt6, self).__init__()
        self.top_k = top_k
        self.correct_count = 0
        #self.total_count = 0
        #  metric name needs to be defined
    ...
    def __call__(self, y_pred: Tensor, y_true: Tensor) -> np.ndarray:
       ...
       # list_pred = [p.item() for p in total_count]
     
        #train_df = self.train_df
        #df_preds = pd.DataFrame(total_count, columns = ['preds_col'])
       ...
       # [[tn_black , fp_black],[fn_black , tp_black]]  = confusion_matrix(df_black['two_year_recid'], df_black['preds_col'])
        ...
        #Fairness_False_positive_rate_black_Caucasian = (fp_black/(fp_black+tn_black)) - (fp_Caucasian/(fp_Caucasian+tn_Caucasian))
        return np.array(total_count[3].item()) #df_preds[2] # np.array(total_count[2]) #Fairness_False_positive_rate_black_Caucasian


class tt7(Metric):
    def __init__(self, top_k: int = 1):
        super(tt7, self).__init__()
        self.top_k = top_k
        self.correct_count = 0
        #self.total_count = 0
        #  metric name needs to be defined
    ...
    def __call__(self, y_pred: Tensor, y_true: Tensor) -> np.ndarray:
       ...
       # [[tn_black , fp_black],[fn_black , tp_black]]  = confusion_matrix(df_black['two_year_recid'], df_black['preds_col'])
        ...
        #Fairness_False_positive_rate_black_Caucasian = (fp_black/(fp_black+tn_black)) - (fp_Caucasian/(fp_Caucasian+tn_Caucasian))
        return np.array(list_pred[1]) #df_preds[2] # np.array(total_count[2]) #Fairness_False_positive_rate_black_Caucasian



class tt8(Metric):
    def __init__(self, top_k: int = 1):
        super(tt8, self).__init__()
        self.top_k = top_k
        self.correct_count = 0
        #self.total_count = 0
        #  metric name needs to be defined
    ...
    def __call__(self, y_pred: Tensor, y_true: Tensor) -> np.ndarray:
        ...
        list_pred = [np.array(p.item()) for p in total_count]
        df_preds = pd.DataFrame(list_pred, columns = ['preds_col'])
       ...
       # [[tn_black , fp_black],[fn_black , tp_black]]  = confusion_matrix(df_black['two_year_recid'], df_black['preds_col'])
        ...
        #Fairness_False_positive_rate_black_Caucasian = (fp_black/(fp_black+tn_black)) - (fp_Caucasian/(fp_Caucasian+tn_Caucasian))
        return list_pred[1] #df_preds[2] # np.array(total_count[2]) #Fairness_False_positive_rate_black_Caucasian
...
class tt9(Metric):
    ...
    def __call__(self, y_pred: Tensor, y_true: Tensor) -> np.ndarray:
        ...
        list_pred = [np.array(p.item()) for p in total_count]
        df_preds = pd.DataFrame(list_pred, columns = ['preds_col'])
        df_preds.to_csv('Output_folder/df_preds_v1_03_07_2022 {} mmm.csv'.format(j), index=False)
        ...
        #Fairness_False_positive_rate_black_Caucasian = (fp_black/(fp_black+tn_black)) - (fp_Caucasian/(fp_Caucasian+tn_Caucasian))
        return list_pred[1] 
        #df_preds[2] # np.array(total_count[2]) #Fairness_False_positive_rate_black_Caucasian
